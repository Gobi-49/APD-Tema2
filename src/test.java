...
class AtomicThread extends Thread {
    ...
    @Override
    public void run() {
        int start = id * (int) Math.ceil((double) test.size / test.noThreads);
        int end = Math.min(test.size, (id + 1) * (int) Math.ceil((double) test.size / test.noThreads));
        for (int i = start; i < end; i++) {
            sum.getAndAdd(test.arr[i]);
        }
    }
}
